...
The R Markdown source for this document is found at
<https://github.com/IRSAAtUMn/RWorkshop17/blob/master/06-rmark.Rmd>.
...
```{r fig.align='center', label="cjgeyer.hist"}
...
```
   to run R to do the analysis, or
...
(Actually there is
a [small example in the following chapter](<git-version-control.html#plethora>),
but we won't bother with it now.)
...
```{r fig.align='center', fig.cap='Simple Linear Regression', label="cjgeyer-simple"}
plot(x, y)
abline(out1)
```
...
```{r fig.cap="Scatter Plot with Cubic Regression Curve", echo=FALSE, fig.align='center', label="cjgeyer-hide"}
plot(x, y)
